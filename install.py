#!/usr/bin/env python3
"""Install the autochess command for the current user on macOS or Linux."""
...
HEADER = '#!/bin/sh\n# Installed by terminal-autochess/install.py\n'
...
def install_launcher(bin_dir):
    ...
    temporary = None
    try:
        with tempfile.NamedTemporaryFile(mode='w', encoding='utf-8', prefix='.autochess-',
...
def main(argv=None):
    parser = argparse.ArgumentParser(description='安装 autochess 短命令（macOS / Linux，无需 sudo）')
